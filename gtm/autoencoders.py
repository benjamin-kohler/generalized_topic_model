...
class AutoEncoderSAGE(nn.Module):
    def __init__(
        ...
        if content_covariate_size != 0:
            ...
            if estimate_interactions:
                self.beta_ci = nn.Linear(
                    content_covariate_size * self.n_topic, self.vocab_size, bias=False
                )
            ...
            if self.estimate_interactions:
                ...
                eta += self.beta_ci(
                    covar_interactions.reshape(
                        (batch_size, self.n_topic * covariate_size)
                    )
                )
        return eta
    ...
    def sparsity_loss(self, l1_beta, l1_beta_c, l1_beta_ci, device):
        ...
        if self.content_covariate_size != 0:
            ...
            sparsity_loss += (
                self.l1_beta_c_reg * (l1_strengths_beta_c * beta_c_weights_sq).sum()
            )
            ...
            if self.estimate_interactions:
                ...
                sparsity_loss += (
                    self.l1_beta_ci_reg
                    * (l1_strengths_beta_ci * beta_ci_weights_sq).sum()
                )
