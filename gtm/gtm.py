    ...
    def epoch(
        ...
        for iter, data in enumerate(data_loader):
            ...
            if (iter + 1) % print_every == 0:
                if validation:
                    print(
                        f"Epoch {(epoch+1):>3d}\tIter {(iter+1):>4d}\tTotal Validation Loss:{loss.item()/len(x_input):<.7f}\tRec Loss:{reconstruction_loss.item()/len(x_input):<.7f}\tMMD Loss:{mmd_loss.item()*w_prior/len(x_input):<.7f}\tSparsity Loss:{decoder_sparsity_loss/len(x_input):<.7f}\tPred Loss:{prediction_loss*w_pred_loss/len(x_input):<.7f}"
                    )
                else:
                    print(
                        f"Epoch {(epoch+1):>3d}\tIter {(iter+1):>4d}\tTotal Training Loss:{loss.item()/len(x_input):<.7f}\tRec Loss:{reconstruction_loss.item()/len(x_input):<.7f}\tMMD Loss:{mmd_loss.item()*w_prior/len(x_input):<.7f}\tSparsity Loss:{decoder_sparsity_loss/len(x_input):<.7f}\tPred Loss:{prediction_loss*w_pred_loss/len(x_input):<.7f}"
                    )

        if validation:
            print(
                f"Epoch {(epoch+1):>3d}\tValidation Loss:{sum(epochloss_lst)/len(epochloss_lst):<.7f}"
            )
        else:
            print(
                f"Epoch {(epoch+1):>3d}\tTraining Loss:{sum(epochloss_lst)/len(epochloss_lst):<.7f}"
            )
